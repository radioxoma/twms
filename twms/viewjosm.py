...
def maps_xml(pretty=True):
    ...
    for layer_id, layer in config.layers.items():
        ...
        ET.SubElement(entry, 'valid-georeference').text = 'true'  # Don't annoy with banner
        ...
        if 'max_zoom' in layer:
            ...
            entry.append(ET.Comment("Overrided with default 'max_zoom' from current TWMS config"))
...
if __name__ == '__main__':
    # print(view_josm_xml(pretty=False))
    print(view_josm_xml(pretty=True))
